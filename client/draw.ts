import { Canvas } from "./domain.js";
import { Vector, vector } from "./vectors.js";
import * as Vec from "./vectors.js";
 
export function drawGrid(c: Canvas, viewOffset: Vector, width: number, height: number,  scale: number){
...
export function draw(c: Canvas, position: Vector, color: string, scale: number){
    c.ctx.beginPath()
